...
export async function POST(request: Request) {
  const body = (await request.json().catch(() => ({}))) as { categoryScope?: unknown; briefLanguage?: unknown; publishDate?: unknown };
  ...
  const options: { categoryScope: string; briefLanguage?: BriefLanguage; publishDate?: string } = {
    categoryScope,
